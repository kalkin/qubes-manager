...
from qubes.qubes import (QubesHost, QubesVmCollection, system_path)
from qubes.qubesutils import (parse_size, updates_dom0_status,
                              updates_dom0_toggle, updates_vms_status,
                              updates_vms_toggle)
...
class GlobalSettingsWindow(Ui_GlobalSettings, QDialog):
    ...
    def __init_system_defaults__(self):
        ...
        all_vms = [vm for vm in self.qvm_collection.values()
                   if not vm.internal and vm.qid != 0]
        self.updatevm_idx = -1
    ...
    def __init_mem_defaults__(self):
        ...
        if self.qmemman_config.has_section('global'):
            self.vm_min_mem_val = self.qmemman_config.get('global',
                                                          'vm-min-mem')
            self.dom0_mem_boost_val = self.qmemman_config.get('global',
                                                              'dom0-mem-boost')
    ...
    def __apply_mem_defaults__(self):
        ...
        if current_min_vm_mem * 1024 * 1024 != self.vm_min_mem_val or \
            ...
            if not self.qmemman_config.has_section('global'):
                ...
                self.qmemman_config.set('global', 'vm-min-mem',
                                        current_min_vm_mem)
                self.qmemman_config.set('global', 'dom0-mem-boost',
                                        current_dom0_mem_boost)
                self.qmemman_config.set('global', 'cache-margin-factor',
                ...
                # SafeConfigParser.write() - it would get rid of all the
                # comments...

                lines_to_add = {}
                lines_to_add[
                    'vm-min-mem'] = "vm-min-mem = " + current_min_vm_mem + "\n"
                lines_to_add['dom0-mem-boost'] = "dom0-mem-boost = " + \
                    current_dom0_mem_boost + "\n"
    ...
    def __apply_updates__(self):
        if self.updates_dom0.isChecked() != self.updates_dom0_val:
            updates_dom0_toggle(self.qvm_collection,
                                self.updates_dom0.isChecked())
        if self.updates_vm.checkState() != Qt.PartiallyChecked:
            self.anything_changed = True
            updates_vms_toggle(self.qvm_collection, self.updates_vm.isChecked())
