...
from qubes.qubes import (QubesHost, QubesVmCollection, QubesVmLabels,
                         system_path)
...
class VMSettingsWindow(Ui_SettingsDialog, QDialog):
    ...
    def __init__(self, vm, app, qvm_collection, init_page="basic", parent=None):
        ...
        self.connect(self.init_mem, SIGNAL("editingFinished()"),
                     self.check_mem_changes)
        self.connect(self.max_mem_size, SIGNAL("editingFinished()"),
                     self.check_mem_changes)
        ...
        self.connect(self.dev_list, SIGNAL("selected_changed()"),
    ...
    def save_and_apply(self):
        ...
        thread = threading.Thread(target=self.__save_changes__, args=(
            thread_monitor, ))
        thread.daemon = True
        thread.start()

        progress = QProgressDialog(
            "Applying settings to <b>{0}</b>...".format(self.vm.name), "", 0, 0)
        ...
        if not thread_monitor.success:
            QMessageBox.warning(
                None,
                "Error while changing settings for {0}!".format(self.vm.name),
        ...
        try:
            if self.tabWidget.isTabEnabled(self.tabs_indices["firewall"]):
                self.fw_model.apply_rules(
                    ...
                    self.dnsCheckBox.isChecked(), self.icmpCheckBox.isChecked(),
    ...
    def current_tab_changed(self, idx):
        if idx == self.tabs_indices["firewall"]:
            if self.vm.netvm is not None and not self.vm.netvm.is_proxyvm():
                QMessageBox.warning(
                    None, "VM configuration problem!",
                    "The '{0}' AppVM is not network connected to a "
                    "FirewallVM!<p>".format(self.vm.name) +
                    "You may edit the '{0}' VM firewall rules, but these will "
                    "not take any effect until you connect it to a working "
                    "Firewall VM.".format(self.vm.name))
    ...
    def __init_basic_tab__(self):
        ...
        if not self.vm.is_template() and self.vm.template is not None:
            template_vm_list = [vm for vm in self.qvm_collection.values()
        ...
        if (not self.vm.is_netvm() or self.vm.is_proxyvm()):
            netvm_list = [vm for vm in self.qvm_collection.values()
        ...
        # installed by rpm
        text = "Yes" if self.vm.installed_by_rpm else "No"
        self.rpm_label.setText(text)

        # networking info
        if self.vm.is_networked():
            self.networking_groupbox.setEnabled(True)
            self.ip_label.setText(self.vm.ip
                                  if self.vm.ip is not None else "none")
            self.netmask_label.setText(self.vm.netmask if self.vm.netmask is
                                       not None else "none")
            self.gateway_label.setText(self.vm.netvm.gateway
                                       if self.vm.netvm is not None else "none")
        else:
    ...
    def __apply_basic_tab__(self):
        ...
        if self.vm.name != vmname:
            if self.vm.is_running():
                ...
                msg.append(
                    "Can't change VM name - a VM named <b>{0}</b> already "
                    "exists in the system!".format(vmname))
            else:
                try:
        ...
        try:
            if self.template_name.currentIndex() != self.template_idx:
                ...
                template_vm = self.qvm_collection.get_vm_by_name(
                    new_template_name)
                assert (template_vm is not None and
                        template_vm.qid in self.qvm_collection)
        ...
        try:
            if self.netVM.currentIndex() != self.netvm_idx:
                ...
                if new_netvm_name == "default":
                    new_netvm_name = self.qvm_collection.get_default_netvm(
                    ).name
                ...
                assert (netvm is None or
                        (netvm is not None and netvm.qid in self.qvm_collection
                         and netvm.is_netvm()))
    ...
    def check_mem_changes(self):
        if self.max_mem_size.value() < self.init_mem.value():
            QMessageBox.warning(
                None, "Warning!",
                "Max memory can not be less than initial memory.<br>Setting "
                "max memory to equal initial memory.")
            self.max_mem_size.setValue(self.init_mem.value())
        # Linux specific limit: init memory must not be below max_mem_size/10.79
        # in order to allow scaling up to max_mem_size (or else "add_memory()
        # failed: -17" problem)
        if self.init_mem.value() * 10 < self.max_mem_size.value():
            QMessageBox.warning(
                None, "Warning!",
                "Initial memory can not be less than one tenth Max memory."
    ...
    def __init_advanced_tab__(self):
        ...
        self.include_in_balancing.setChecked(self.vm.services['meminfo-writer'])
        if self.vm.type == "HVM":
        ...
        if not hasattr(self.vm, "kernel"):
            ...
            for (i, k) in enumerate(kernel_list):
                ...
                if (text.startswith("default") and self.vm.uses_default_kernel
                    ) or (self.vm.kernel == k and
                          not self.vm.uses_default_kernel) or (
                              k == "none" and self.vm.kernel is None):
                    text += " (current)"
                    self.kernel_idx = i
        ...
        if not hasattr(self.vm, "drive"):
            ...
            vm_list = [vm for vm in self.qvm_collection.values()
    ...
    def __apply_advanced_tab__(self):
        ...
        if hasattr(self.vm, "drive") and self.drive_groupbox.isVisible():
            try:
                if not self.drive_groupbox.isChecked():
                    ...
                    if self.drive_domain.currentIndex(
                    ) == self.drive_domain_idx:
                        # strip "(current)"
                        new_domain = new_domain.split(' ')[0]
                    drive = "%s:%s:%s" % (str(self.drive_type.currentText()),
                                          new_domain,
        ...
        try:
            ...
            if self.dispvm_netvm.currentIndex() != self.dispvm_netvm_idx:
                ...
                assert (dispvm_netvm is None or
                        (dispvm_netvm.qid in self.qvm_collection and
                         dispvm_netvm.is_netvm()))
    ...
    def __init_devices_tab__(self):
        ...
        for d in devs:
            if d[1] in self.vm.pcidevs:
                self.dev_list.selected_list.addItem(DevListWidgetItem(d[0], d[
                    1]))
            else:
                self.dev_list.available_list.addItem(DevListWidgetItem(d[0], d[
                    1]))

        if self.dev_list.selected_list.count(
        ) > 0 and self.include_in_balancing.isChecked():
    ...
    def __apply_devices_tab__(self):
        ...
        try:
            ...
            if self.dev_list.selected_list.count() - len(added) < len(
                    self.vm.pcidevs):  # sth removed
                sth_changed = True
            elif len(added) > 0:
                sth_changed = True
    ...
    def __init_services_tab__(self):
        for srv in self.vm.services:
            ...
            if self.vm.services[srv]:
                item.setCheckState(QtCore.Qt.Checked)
            else:
                item.setCheckState(QtCore.Qt.Unchecked)
            self.services_list.addItem(item)

        self.connect(self.services_list,
    ...
    def __remove_service__(self):
        ...
        if str(item.text()) == 'meminfo-writer':
            QMessageBox.information(
                None, "Service can not be removed",
                "Service meminfo-writer can not be removed from the list.")
            return
    ...
    def __apply_services_tab__(self):
        ...
        try:
            ...
            balancing_was_checked = (self.vm.services['meminfo-writer'])
            balancing_is_checked = self.include_in_balancing.isChecked()
            meminfo_writer_checked = (self.new_srv_dict['meminfo-writer'])
    ...
    def delete_rule_button_pressed(self):
        selected_indexes = self.rulesTreeView.selectedIndexes()
        for i in set([index.row() for index in selected_indexes]):
        ...
        if dialog.exec_():
            ...
            if port is not None:
                if port2 is not None and port2 <= port:
                    QMessageBox.warning(
                        None, "Invalid service ports range",
...
def handle_exception(exc_type, exc_value, exc_traceback):
    ...
    msg_box.setText(
        ...
        "<b><i>%s</i></b>" % error +
        "<br/>at line <b>%d</b><br/>of file %s.<br/><br/>" % (line, filename))
    ...
    if len(sys.argv) > 1:
        ...
        if vm is None or vm.qid not in qvm_collection:
            QMessageBox.critical(
                ...
                "A VM with the name '{0}' does not exist in the system.".format(
                    sys.argv[1]))
            sys.exit(1)
        if len(sys.argv) > 2:
            tab_arg = sys.argv[2]
            ...
            if tab_arg in VMSettingsWindow.tabs_indices:
                ...
                QMessageBox.warning(
                    None, "Qubes VM Settings Error",
                    "There is no such tab as '{0}'. "
                    "Opening default tab instead.".format(tab_arg))

    else:
        vms_list = [vm.name for vm in qvm_collection.values()
                    if (vm.is_appvm() or vm.is_template())]
        vmname = QInputDialog.getItem(None, "Select VM", "Select VM:", vms_list,
                                      editable=False)
        if not vmname[1]:
            sys.exit(1)
