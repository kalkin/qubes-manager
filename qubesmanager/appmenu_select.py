...
class AppmenuSelectManager:
    ...
    def fill_apps_list(self):
        ...
        whitelisted = [a for a in whitelisted
                       if os.path.exists('%s/apps/%s-%s' % (self.vm.dir_path,
                                                            self.vm.name, a))]
        ...
        for template_file in template_file_list:
            ...
            if not desktop_command:
                desktop_command = ""
            if desktop_name:
                available_appmenus.append(
                    (template_file, desktop_name, desktop_command))
            desktop_template.close()

        self.whitelisted_appmenus = [a for a in available_appmenus
                                     if a[0] in whitelisted]
        available_appmenus = [a for a in available_appmenus
                              if a[0] not in whitelisted]

        for a in available_appmenus:
            self.app_list.available_list.addItem(
                AppListWidgetItem(a[1], a[0], a[2]))

        for a in self.whitelisted_appmenus:
            self.app_list.selected_list.addItem(
                AppListWidgetItem(a[1], a[0], a[2]))
    ...
    def save_list_of_selected(self):
        ...
        if self.app_list.selected_list.count() - len(added) < \
                len(self.whitelisted_appmenus):  # sth removed
            sth_changed = True
        elif len(added) > 0:
            sth_changed = True

        if sth_changed:
            whitelisted = open(self.vm.dir_path + '/' + whitelisted_filename,
                               'w')
            for i in range(self.app_list.selected_list.count()):
                item = self.app_list.selected_list.item(i)
